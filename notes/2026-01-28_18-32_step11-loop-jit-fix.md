...
During debugging, discovered that Kujo's `StoreVar` opcode PEEKS (doesn't pop) the stack. This is documented in GOTCHAS.md but the comment there was misleading.
...
4. **tests/jit_loop_tests.kujo**: Created comprehensive loop test file

## Tests

- All 31 JIT tests passing
- `test_compile_simple_loop` now passing (was previously failing)
- `tests/jit_loop_tests.kujo` runs correctly with all 5 tests passing
...
2. **Assignment syntax**: Kujo uses `:=` for assignment, not `=`. The `=` operator is for equality or something else. This caused an infinite loop in my initial while loop test.
