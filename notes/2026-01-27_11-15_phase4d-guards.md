# Kujo Field Notes — Phase 4D: Guard Generation for Specialized Code
...
- Guards are NOT a performance optimization — they're a **safety mechanism**
- Specialized code makes TYPE ASSUMPTIONS (e.g., "variable X is always an int")
- Guards CHECK those assumptions at function entry
- If assumptions hold → run fast code
- If assumptions broken → bail out safely
...
- Checking once at entry is sufficient (types don't change mid-function in Kujo)
...
1. **Without guards**: Can't specialize because types might change → must use generic slow path
...
Kujo now has this capability. Phase 4D is complete. 🎉
