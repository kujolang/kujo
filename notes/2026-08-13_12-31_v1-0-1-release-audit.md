...
`v1.0.1` satisfies the repository's official binary-release artifact contract for Linux x64, macOS x64, macOS arm64, and Windows x64. The release is complete and installable for the supported platforms.

## Post-Audit Tag Signing

On 2026-08-13, the annotated `v1.0.1` tag was recreated with an SSH signature while preserving its original target commit, tagger identity, tagger timestamp, and message. GitHub reports tag object `36079145709aa565e0ca638f0e9d5ddef640083c` as `verified: true` with reason `valid`. The release remained attached to `v1.0.1`, and all nine original release assets retained their asset IDs, timestamps, sizes, and URLs.

The tag update caused the tag-triggered workflows to enqueue. The redundant `release-binaries` run `31722640657` was cancelled before builds or release publication could run, preventing replacement of the already-verified assets. The unrelated `kujo-tool-artifacts-guard` repeated the known tag-commit ignore-rule failure described above.
