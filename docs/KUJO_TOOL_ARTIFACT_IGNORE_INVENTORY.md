...
| Muzzle | `.muzzle/logs/`, `.muzzle/reports/`, `.muzzle/state/`, `.muzzle/.kujo_cache/`, `.muzzle/manifests/`, `.muzzle/workflows/`, `.dogfood/` | `state/`, `.kujo_cache/`, logs, and reports are local run state. `manifests/` and `workflows/` can be tracked examples. |
...
| Kujo workflow runners | `.runs/<timestamp>/`, `.work/<timestamp>/`, `.kujo/runs/<run-id>/`, `.kujo/feature-cards/<card-id>/`, `.kujo/agency/auth/<site>/<role>.storage-state.json`, `.kujo/pi/`, `.health.json`, `.login.log` | Workflow kits use these roots for local proof packets, disposable workspaces, resumable run state, handoffs, browser/login evidence, and logs. Kujo Pi creates `.kujo/pi/README.md` with `init` and defaults MCP/Dispatch artifacts to `.kujo/pi/mcp` and `.kujo/pi/dispatch`; track only deliberate project setup. `.kujo/agency/sites/*.yml` profiles are caller-authored config and should be tracked or ignored per repository policy rather than covered by the shared block. |
...
| Email | `.email/` default state directory including `email.db`, SQLite sidecars, message blobs, outbound wires, evidence, provider evidence, HTTP upload spools, SMTP spools/quarantine, generated credential/key files, and TLS material; `.email-test*/` fixture state roots | `KUJO_EMAIL_STATE_DIR` defaults to `.email`; examples and operations docs write mode-0600 secrets under that root. Shared `.kujo/packs/email/` Doctor pack files are source-worthy and intentionally not covered by the `.kujo/pi/` rule. |
...
| AI SDK | `artifacts/security/integrity-manifest.sha256`, `artifacts/hardening/` logs, generated catalogs, wrapper measurements, stress output, integrity manifests, and other local release/security artifacts | CI may upload reviewed release artifacts; local hardening evidence is documented as ignored and not shipped. |
| Ability Gateway | `artifacts/live-conformance.json` from live OAuth/gateway certification | The runner writes a mode-0600 redacted receipt while keeping tokens, authorization codes, OAuth state, GitHub credentials, and request bodies out of the file. |
...
- `/.email/` and `/.email-test*/` cover Kujo Email's default operator state, generated secrets, retained evidence/spools, SQLite store, and local fixture state while leaving shared Doctor packs under `.kujo/packs/email/` trackable.
- `/data/*.db`, `/data/*.db-shm`, `/data/*.db-wal`, `/data/*.db-journal`, `/data/*.sqlite-journal`, `/data/*.sqlite3-journal`, `/data/*.log`, `/*.db-shm`, `/*.db-wal`, `/*.db-journal`, `/*.sqlite`, `/*.sqlite-journal`, `/*.sqlite3`, and `/*.sqlite3-journal` cover SQLite/log sidecars emitted by Watchdog, AI Chat, CMS, CRUD API, RAG, Email when the state root is caller-selected, and related local state.
...
- `/artifacts/hardening/` covers AI SDK local hardening logs and generated measurement artifacts; `/artifacts/release/` and `/artifacts/browser/` cover SiteKit release archives/checksums and Playwright reports while avoiding a blanket `/artifacts/` rule.
- `/artifacts/live-conformance.json` covers Ability Gateway's local live-certification receipt without ignoring reviewed evidence docs.
