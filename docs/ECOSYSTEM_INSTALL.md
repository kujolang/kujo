...
```bash
curl -fsSL https://raw.githubusercontent.com/kujolang/kujo/main/install.sh | bash -s -- --ref v1.0.2
...
Kujo's tagged binaries, checksum files, and published-artifact smokes are the canonical ecosystem onboarding path. Treat `main` as development state; use the immutable `v1.0.2` tag or a later supported release tag for reproducible release installs.
