...
| `http_server` | `http_server(...)` | handler-defined | dynamic (Value) | Creates a routed server with an 8 MiB pre-dispatch body bound and a socket read deadline; oversized requests receive 413 and incomplete reads receive 408. Request dictionaries include trustworthy socket-derived `peer_address`, `peer_ip`, `peer_port`, and `peer_transport` fields. | `none` | `result := http_server(...)` |
