...
| `env_int` | `env_int(name: string) -> int` | handler-defined | int | `Value::ErrorObject` when missing or not a valid integer; restricted runs require `env-read`. | `env-read` | `result := env_int("PORT")` |
| `env_float` | `env_float(name: string) -> float` | handler-defined | float | `Value::ErrorObject` when missing or not a valid float; restricted runs require `env-read`. | `env-read` | `result := env_float("TIMEOUT")` |
| `env_bool` | `env_bool(name: string) -> bool` | handler-defined | bool | `Value::ErrorObject` when missing or not one of `true/1/yes/on` or `false/0/no/off` (case-insensitive); restricted runs require `env-read`. | `env-read` | `result := env_bool("ENABLED")` |
