...
| `read_stdin` | `read_stdin(max_bytes)` | exact 1 | string | Error on invalid limit, overflow, I/O, or invalid UTF-8. | `none` | `request := read_stdin(1048576)` |
...
| `list_dir_beneath` | `list_dir_beneath(root, relative_directory, after, suffix, limit, max_entries)` | exact 6 | dict | Returns bounded names, next_cursor, truncated and scanned; rejects symlink/reparse traversal, invalid paths/UTF-8, limits outside 1..1000 and scan ceilings outside 1..100000, or exceeding the scan ceiling. | `filesystem-read` | `page := list_dir_beneath(".", "records", "", ".json", 100, 100000)` |
...
| `sync_directory_beneath` | `sync_directory_beneath(root, relative_directory)` | exact 2 | bool | Error on unsupported platform, rejected path or unconfirmed directory sync. | `filesystem-write` | `sync_directory_beneath(".", "state")` |
...
| `list_dir_page` | `list_dir_page(path, after, limit, suffix)` | exact 4 | dictionary | UTF-8 filenames greater than exclusive filename cursor, filtered by suffix, sorted lexically. Limit 1..10000. Returns entries, next_after, truncated, examined_entries and buffered_entries. Scans the directory with at most limit+1 retained names; no snapshot across calls. Entry errors fail closed. | `filesystem-read` | `page := list_dir_page("records", "", 100, ".json")` |
...
| `decode_uri_component` | `decode_uri_component(...)` | handler-defined | string | Value::Error on malformed escapes, invalid UTF-8, or invalid args/types. | `none` | `text := decode_uri_component("caf%C3%A9")` |
...
| `secure_random_token` | `secure_random_token(byte_length: int) -> Secret` | exact 1 | Secret | Generates 16–128 bytes with the operating-system CSPRNG, encodes them as unpadded base64url, and is never affected by `set_random_seed`. The returned value is redacted until explicitly passed to `reveal`; entropy failures fail closed. | `random` | `token := secure_random_token(32)` |
...
| `format_date_tz` | `format_date_tz(timestamp: number, format: string, timezone: string) -> string` | exact 3 | string | Value::ErrorObject for invalid timestamps or IANA time-zone identifiers. Applies daylight-saving transitions. | `clock` | `local := format_date_tz(now_unix(), "YYYY-MM-DD HH:mm", "America/Detroit")` |
...
| `parse_datetime` | `parse_datetime(value: string) -> float` | exact 1 | float | Value::ErrorObject unless the value is a valid RFC 3339 timestamp with an offset. Returns Unix seconds. | `clock` | `seconds := parse_datetime("2026-09-16T09:30:00-04:00")` |
...
| `pdf_render_html` | `pdf_render_html(html: string, options: dict, assets: dict) -> dict` | exact 3 | dictionary | Canonicalizes and renders the strict profile in `docs/PDF_RENDERING.md` entirely in-process. Returns a receipt containing `pdf_bytes`, SHA-256 digests, page geometry/count, renderer versions, deterministic warning codes, and duration. Input cannot access the network or filesystem. Tables can opt into deterministic header repetition with bounded `data-repeat-header-every`. | `none` | `receipt := pdf_render_html("<h1>Estimate</h1>", {"page_size":"Letter"}, {})` |
| `pdf_render_html_to_file` | `pdf_render_html_to_file(html: string, options: dict, assets: dict, destination: string) -> dict` | exact 4 | dictionary | Uses the same renderer, then atomically publishes a mode-0600 file without overwriting. Destination must be absolute and its parent must exist. | `filesystem-write` | `receipt := pdf_render_html_to_file(html, {}, {}, "/private/quotes/Q-1007.pdf")` |
...
| `http_download_file` | `http_download_file(url, output_path, options)` | exact 3 | dictionary | Streams to a same-directory temporary file, enforces `max_bytes`, verifies a SHA-256 digest, and atomically publishes only complete responses. `timeout_ms` is bounded to 1..300000; `destination_policy: "deny_private"`, `pin_dns: true`, and `redirects: "none"` provide a strict per-request destination contract. | `network-client`, `filesystem-write` | `receipt := http_download_file(url, "artifact.bin", {"max_bytes":1073741824,"destination_policy":"deny_private","pin_dns":true,"redirects":"none"})` |
| `http_upload_file` | `http_upload_file(url, input_path, options)` | exact 3 | dictionary | Streams a file as a POST/PUT body without base64 expansion and bounds the response with `max_response_bytes`. It supports the same bounded `timeout_ms`, strict destination policy, DNS pinning, and redirect controls as `http_download_file`. | `network-client`, `filesystem-read` | `receipt := http_upload_file(url, "artifact.bin", {"method":"PUT","destination_policy":"deny_private","pin_dns":true,"redirects":"none"})` |
...
| `db_connect_postgres_tls` | `db_connect_postgres_tls(connection_string: string, ca_pem: string) -> Database` | exact 2 | Database | Requires a bounded non-empty public CA bundle and explicit TCP host, forces PostgreSQL TLS even if the connection string requests otherwise, enforces TLS 1.2+, trusts only that bundle, verifies the peer hostname, and returns secret-free deterministic failures. | `database` | `db := db_connect_postgres_tls(url, read_file("postgres-ca.pem"))` |
...
| `db_close` | `db_close(db: Database) -> bool` | handler-defined | bool | Releases the native connection and invalidates all aliases; repeated close returns true. Rolls back uncommitted work. Close failures return Value::Error. See `docs/DATABASE_LIFECYCLE.md`. | `database` | `db_close(db)` |
...
| `db_pool_postgres_tls` | `db_pool_postgres_tls(connection_string: string, ca_pem: string, options: dict) -> DatabasePool` | exact 3 | DatabasePool | Creates and warms a bounded PostgreSQL pool that forces TLS 1.2+, verifies hostname and a caller-supplied public CA bundle, bounds connect/acquisition/query time, checks health, expires idle/old connections, resets sessions on return, rejects double/cross-pool release, and exposes operational counters through `db_pool_stats`. Unknown options fail closed. See `docs/POSTGRES_TLS_POOL.md`. | `database` | `pool := db_pool_postgres_tls(url, read_file("postgres-ca.pem"), {"min_connections":2,"max_connections":10,"acquisition_timeout_ms":2000,"statement_timeout_ms":30000})` |
...
| `db_begin_immediate` | `db_begin_immediate(db)` | handler-defined | bool | Begins a SQLite `IMMEDIATE` transaction, waiting according to the configured busy timeout before failing; rejects non-SQLite databases, nested transactions, invalid arguments and capability denial. | `database` | `db_begin_immediate(db)` |
...
| `copy_file_beneath` | `copy_file_beneath(source_root: string, source_path: string, target_root: string, target_path: string, max_bytes: int) -> dict` | exact 5 | `{sha256, bytes}` | Streams a regular no-follow source through a fixed 64 KiB buffer to an atomic, no-replace destination beneath retained directory handles. Bounds 1..4 GiB; rejects oversize metadata and growth. Failed writes discard the temporary. Roots are trusted; relative components cannot traverse links. Receipt covers copied bytes, not a source snapshot. | `filesystem-read`, `filesystem-write` | `copy_file_beneath("assets", "clip.mp4", "backup", "clip.mp4", 4294967296)` |
| `sha256_file_beneath` | `sha256_file_beneath(root: string, relative_path: string, max_bytes: int) -> dict` | exact 3 | `{sha256, bytes}` | Confined no-follow regular-file hashing with a fixed 64 KiB buffer and a 1..4 GiB ceiling; rejects oversize metadata and observed growth beyond the ceiling. Root is trusted; relative paths contain only normal components. Returns actual bytes hashed, not a snapshot guarantee. | `filesystem-read` | `digest := sha256_file_beneath("assets", "video.mp4", 4294967296)` |
...
| `decode_text_file_range_info` | `decode_text_file_range_info(path, offset, count, transfer_encoding, charset, max_output_bytes, prefix_bytes)` | exact 7 | string, int, int, string, string, int, int | Streams an exact file range through strict transfer decoding and then strict registered-charset conversion to UTF-8. Both decoded octets and UTF-8 output share the explicit 64 MiB ceiling; returns hashes, sizes, NUL/ASCII facts and only a caller-capped UTF-8 prefix (maximum 4096 bytes). | `filesystem-read` | `info := decode_text_file_range_info(path, 0, size, "quoted-printable", "iso-8859-1", 1048576, 64)` |
...
| `aes_encrypt_file_stream` | `aes_encrypt_file_stream(input_path, output_path, key, chunk_size)` | exact 4 | dictionary | Constant-memory framed AES-256-GCM with authenticated order, lengths, final frame, SHA-256 receipt, mode-0600 creation on Unix, and atomic no-replace output publication. Receipts add published and temporary_removed; a false cleanup fact includes temporary_path for operator cleanup and never reports a published result as a failed operation. | `filesystem-read`, `filesystem-write` | `receipt := aes_encrypt_file_stream("in.bin", "out.aead", key, 1048576)` |
| `aes_decrypt_file_stream` | `aes_decrypt_file_stream(input_path, output_path, key)` | exact 3 | dictionary | Authenticates every frame before writing, creates output mode 0600 on Unix, and removes temporary output on truncation, tampering, reordering, or format failure. | `filesystem-read`, `filesystem-write` | `receipt := aes_decrypt_file_stream("out.aead", "restored.bin", key)` |
...
| `ip_classify` | `ip_classify(address)` | exact 1 | dictionary | Deterministically normalizes and classifies an IP literal as global, private, loopback, link-local, multicast, documentation, shared, benchmark, mapped, reserved, or unspecified; reports fail-closed public routability plus canonical `address` and fully expanded lowercase `expanded_address`. | `none` | `result := ip_classify("192.0.2.1")` |
...
### Bounded web-data and artifact mechanisms

These additive APIs support native Kujo tools. Crawling, robots rules, retries,
artifact schemas, and report policy belong in Kujo packages, not these primitives.
They raise inspectable runtime errors on invalid input or bounds violations.

| Function | Signature | Arity | Returns | Contract and errors | Capability | Example |
| --- | --- | --- | --- | --- | --- | --- |
| `html_tokens` | `html_tokens(text, max_events)` | exact 2 | array | html5ever HTML tokenizer events: `kind`, `name`, `attrs`, `self_closing`, or text. UTF-8 input ≤8 MiB, 1–1,000,000 events; adjacent text events merge. Comments/doctype are omitted. Script/style raw text is preserved. Invalid arguments/capability denial raise runtime errors. | `none` | `result := html_tokens("<title>Hi</title>", 100)` |
| `url_normalize` | `url_normalize(url, base, options)` | exact 3 | string | HTTP(S), no credentials/ASCII controls, no fragment. WHATWG host/path resolution, default ports removed. `query_mode`: preserve/sort/drop; `deny_params`: case-insensitive names. Invalid URLs return empty string; invalid options raise. Invalid arguments/capability denial raise runtime errors. | `none` | `result := url_normalize("/docs", "https://example.com", {})` |
| `url_components` | `url_components(url)` | exact 1 | dictionary | Structured scheme, host, effective port, origin, path, query. Invalid arguments/capability denial raise runtime errors. | `none` | `result := url_components("https://example.com/docs")` |
| `decode_text_lossy` | `decode_text_lossy(bytes, charset, max_bytes)` | exact 3 | string | Replacement decoding; unknown charset uses UTF-8. Input and explicit UTF-8 output bound ≤512 MiB. Latin-1 retains its ISO byte mapping. Invalid arguments/capability denial raise runtime errors. | `none` | `result := decode_text_lossy(bytes([65]), "utf-8", 1024)` |
| `http_destination_check` | `http_destination_check(url)` | exact 1 | dictionary | Resolve and evaluate public-network policy without sending an HTTP request; `{ok,error}`. Invalid arguments/capability denial raise runtime errors. | `network-client` | `result := http_destination_check("https://example.com")` |
| `http_get_file_response` | `http_get_file_response(url, path, options)` | exact 3 | dictionary | GET streamed to a new file, including non-success status responses. Returns status, headers, bytes, path. Explicit `max_bytes` ≤512 MiB; `redirects: "none"` required. Supports file-HTTP timeout_ms, headers, pin_dns, destination_policy. Atomic no-replace publication; failures remove temporary output. Invalid arguments/capability denial raise runtime errors. | `network-client`, `filesystem-write` | `result := http_get_file_response("https://example.com/sitemap.xml", "sitemap.xml", {"max_bytes":1048576,"redirects":"none","pin_dns":true})` |
| `xml_file_select` | `xml_file_select(path, options)` | exact 2 | dictionary | Streaming XML local-name text projection: `{root,values,truncated}`. Options: element, max_matches (≤1,000,000), max_bytes (≤512 MiB expanded), compression none/gzip. Depth ≤64; selected text ≤32 MiB. Rejects DTD, malformed XML, excess expansion; never resolves external entities. Invalid arguments/capability denial raise runtime errors. | `filesystem-read` | `result := xml_file_select("sitemap.xml", {"element":"loc","max_matches":1000,"max_bytes":1048576})` |
| `text_file_read` | `text_file_read(path, max_bytes)` | exact 2 | string | Strict UTF-8 from a bounded regular non-symlink file ≤512 MiB. Invalid arguments/capability denial raise runtime errors. | `filesystem-read` | `result := text_file_read("report.md", 1048576)` |
| `json_file_read` | `json_file_read(path, max_bytes)` | exact 2 | JSON value | Regular non-symlink JSON file ≤512 MiB, JSON nesting ≤64. Independent of the existing 8 MiB parse_json limit. Invalid arguments/capability denial raise runtime errors. | `filesystem-read` | `result := json_file_read("data.json", 1048576)` |
| `json_file_write` | `json_file_write(path, value, max_bytes)` | exact 3 | bool | Pretty JSON + newline, ≤512 MiB; atomic no-replace file publication. Invalid arguments/capability denial raise runtime errors. | `filesystem-write` | `result := json_file_write("data.json", {"ok":true}, 1048576)` |
| `jsonl_read_chunk` | `jsonl_read_chunk(path, offset, max_rows, max_bytes)` | exact 4 | dictionary | Regular JSONL file ≤512 MiB; explicit byte offset; ≤100,000 rows and bounded batch bytes. Returns rows, offset, eof. Oversized individual records fail. Read a stable file; concurrent writes are not a snapshot. Invalid arguments/capability denial raise runtime errors. | `filesystem-read` | `result := jsonl_read_chunk("pages.jsonl", 0, 64, 1048576)` |
| `jsonl_write` | `jsonl_write(path, rows, append, max_bytes)` | exact 4 | bool | New/append regular JSONL file with total byte cap ≤512 MiB. An error may leave partial staged output; caller owns staging cleanup. Invalid arguments/capability denial raise runtime errors. | `filesystem-write` | `result := jsonl_write("pages.jsonl", [{"url":"https://example.com"}], false, 1048576)` |
| `jsonl_sort` | `jsonl_sort(source, destination, key_fields, buffer_bytes)` | exact 4 | bool | External JSONL sort, string keys, bounded chunks, adaptive 2–32 input merge fan-in. Memory includes up to one oversized record per input and decoded keys. No replacement; temporary files cleaned on error. Invalid arguments/capability denial raise runtime errors. | `filesystem-write`, `filesystem-read` | `result := jsonl_sort("pages.jsonl", "sorted.jsonl", ["url"], 65536)` |
| `jsonl_wrap_array` | `jsonl_wrap_array(source, destination, field, metadata, max_bytes)` | exact 5 | bool | Stream validated JSONL records into one JSON object's array; metadata must not contain field. Input/output bound ≤512 MiB; no replacement. Invalid arguments/capability denial raise runtime errors. | `filesystem-write`, `filesystem-read` | `result := jsonl_wrap_array("pages.jsonl", "pages.json", "pages", {}, 1048576)` |
| `regular_file_digest` | `regular_file_digest(path, max_bytes)` | exact 2 | dictionary | SHA-256 and byte count through the same bounded regular-file handle. Reject symlinks; Unix no-follow + identity checks. Invalid arguments/capability denial raise runtime errors. | `filesystem-read` | `result := regular_file_digest("pages.jsonl", 1048576)` |
| `create_temp_dir` | `create_temp_dir(parent, prefix)` | exact 2 | string | Private unique directory. Caller owns cleanup. Prefix cannot contain path separators. Invalid arguments/capability denial raise runtime errors. | `filesystem-write` | `result := create_temp_dir(".", ".stage-")` |
| `publish_directory_noreplace` | `publish_directory_noreplace(source, destination)` | exact 2 | bool | Atomic directory rename that never replaces a destination. macOS/Linux/Windows supported; other systems fail closed. Invalid arguments/capability denial raise runtime errors. | `filesystem-write`, `filesystem-delete` | `result := publish_directory_noreplace(".stage", "run")` |
| `rate_limit_wait` | `rate_limit_wait(channel, interval_ms)` | exact 2 | bool | Serialize access to a dedicated channel, waiting until a monotonic interval elapsed. 0–60,000 ms, finite; do not use the channel for other messages. Invalid arguments/capability denial raise runtime errors. | `clock` | `result := rate_limit_wait(channel(), 0)` |
| `process_usage` | `process_usage()` | exact 0 | dictionary | cpu_seconds and peak_rss_bytes for this process on Unix; null values on unsupported platforms. Invalid arguments/capability denial raise runtime errors. | `clock` | `result := process_usage()` |
