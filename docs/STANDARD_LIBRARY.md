...
| `http_server` | `http_server(...)` | handler-defined | dynamic (Value) | Creates a routed server whose inbound bodies are read with the global 8 MiB network bound; oversized requests receive 413 before route dispatch. | `none` | `result := http_server(...)` |
| `http_listen` | `http_listen(...)` | handler-defined | dynamic (Value) | Creates a host-bound routed server with the same bounded-body and 413 behavior as `http_server`; listening is capability-gated. | `network-server` | `result := http_listen(...)` |
