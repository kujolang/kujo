...
| `db_pool_postgres_tls` | `db_pool_postgres_tls(connection_string: string, ca_pem: string, options: dict) -> DatabasePool` | exact 3 | DatabasePool | Creates and warms a bounded PostgreSQL pool that forces TLS 1.2+, verifies hostname and a caller-supplied public CA bundle, bounds connect/acquisition/query time, checks health, expires idle/old connections, resets sessions on return, rejects double/cross-pool release, and exposes operational counters through `db_pool_stats`. Unknown options fail closed. | `database` | `pool := db_pool_postgres_tls(url, read_file("postgres-ca.pem"), {"min_connections":2,"max_connections":10,"acquisition_timeout_ms":2000,"statement_timeout_ms":30000})` |
