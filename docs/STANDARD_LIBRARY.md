...
| `db_close` | `db_close(db: Database) -> bool` | exact 1 | bool | Releases the native connection and invalidates all aliases; repeated close returns true. Rolls back uncommitted work. Close failures return Value::Error. See `docs/DATABASE_LIFECYCLE.md`. | `database` | `db_close(db)` |
