...
| `pdf_render_html` | `pdf_render_html(html: string, options: dict, assets: dict) -> dict` | exact 3 | dictionary | Renders a strict, bounded HTML/CSS profile to PDF entirely in-process. Returns a receipt containing `pdf_bytes`, SHA-256 digests, page geometry/count, renderer versions, warnings, and duration. Input cannot access the network or filesystem. | `none` | `receipt := pdf_render_html("<h1>Estimate</h1>", {"page_size":"Letter"}, {})` |
